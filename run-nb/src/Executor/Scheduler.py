...
class Scheduler:
    ...
    def __init__(self, config: dict):
        ...
        OUTPUT_CONTROLES_PATH = OUTPUT_PATH + os.path.sep + CONTROLES
        if not os.path.exists(OUTPUT_CONTROLES_PATH):
            logging.info(f'Path  {OUTPUT_CONTROLES_PATH} not exists, creating ...')
            os.makedirs(OUTPUT_CONTROLES_PATH)
        keys = config.keys()
        logging.info(f'Numero de controles: {len(keys)}')
        for key in keys:
            logging.info(f'{key}')
            name = config.get(key).get(NOTEBOOKS)[0]
            path = f'{NOTEBOOKS}/{name}'
            dates = config.get(key).get(DATES)
            for date in dates:
                logging.info(f'{date}')
                OUTPUT_DATE_PATH = OUTPUT_CONTROLES_PATH + os.path.sep + key + os.path.sep + date
                if not os.path.exists(OUTPUT_DATE_PATH):
                    logging.info(f'Path  {OUTPUT_DATE_PATH} not exists, creating ...')
                    os.makedirs(OUTPUT_DATE_PATH)
                logging.info(f'Inicializando notebook {OUTPUT_DATE_PATH}/{name} ...')
                params = [{NAME: '@DATE@', VALUE: date, TYPE: CONSTANT}]
                nb = Notebook(path=path, params=params, kernel_name=PYTHON_3, output_path=OUTPUT_DATE_PATH)
